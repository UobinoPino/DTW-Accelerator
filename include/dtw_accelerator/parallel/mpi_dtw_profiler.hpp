        ...
        namespace mpi {
            ...
            public:
                ...
                void reportToConsole() {
                    ...
                    std::cout << std::setw(15) << std::right << std::fixed << std::setprecision(2) << total_time << " ms" << std::endl;
...
#pragma omp simd
            ...
            inline void run_dtw_profiler_tests(
                ...
                MPIProfiler wavefront_profiler("MPI_Wavefront_DTW");
                auto [cost1, path1] = dtw_mpi_profiled(A, B, wavefront_profiler, comm);

                if (rank == 0) {
                    std::cout << "\nMPI Wavefront DTW result: " << cost1 << std::endl;
                    wavefront_profiler.reportToConsole();
                }

                if (save_to_file) {
                    wavefront_profiler.saveToFile("dtw_wavefront_profile");
                }
                ...
                if (rank == 0) {
                    ...
                    for (const auto& [section, entry] : wavefront_profiler.timings) {
                    ...
                    std::cout << "Wavefront (MPI) implementation: " << std::fixed << std::setprecision(1)
                    ...
                    if (wavefront_time < blocked_time) {
                        std::cout << "The wavefront (MPI) implementation is "
                                  << std::fixed << std::setprecision(1)
                                  << blocked_time / wavefront_time
                                  << "x faster than the sequential implementation." << std::endl;
                    } else {
                        std::cout << "The blocked implementation is "
                                  << std::fixed << std::setprecision(1)
                                  << wavefront_time / blocked_time
                                  << "x faster than the wavefront (MPI) implementation." << std::endl;
                    }
